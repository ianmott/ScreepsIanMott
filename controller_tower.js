        ...
        if (_.some(targets)) { _.forEach(towers, t => t.attack(targets[0])); }
            ...
            if (_.some(targets)) { _.forEach(towers, t => t.attack(targets[0])); }
                ...
                if (_.some(targets)) { while (_.some(towers) && _.some(targets)) { towers.shift().heal(targets.shift()); } }
                    ...
                    else if (_.some(towers) && towers[0].energy > 700 && _.filter(Game.creeps, (creep) => creep.memory.role == 'tharvester').length >0) { targets = _.sortBy(room.find(FIND_STRUCTURES, { filter: s => should.tower.repair(s) }), s => s.hits);  if (_.some(targets)) { while (_.some(towers) && _.some(targets)) { towers.shift().repair(targets.shift()); } }}
                //}
            }
        }
    }
