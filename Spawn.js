    ...
    run: function(RoomName) {
    	...
    	var TWeights = new Array(NumofTypes);
    	   //      Harvester            Guard         Upgrader         repairer        
    	    TWeights[0] = 0; TWeights[1] = 8; TWeights[2] = 3; TWeights[3] = 5;
    	   //      builder       rangedguard        tharvester      atcontroller
    	    TWeights[4] = 6; TWeights[5] = 7; TWeights[6] = 4; TWeights[7] = 9;
    	   //      Gather       Haul
    	   TWeights[8] = 1; TWeights[9] = 2;
    	var toggle = false;
        var Types = new Array(NumofTypes);           // Type Name     Type Min       Type Current            Parts                                                        Cost
        Types[TWeights[0]] = new Array(numofFields); Types[TWeights[0]][0] = 'harvester';    Types[TWeights[0]][1] = 1;  Types[TWeights[0]][4] = 999;  Types[TWeights[0]][2] = _.filter(Game.creeps, (creep) => creep.memory.role == 'harvester' && creep.memory.roomName == RoomName).length;     Types[TWeights[0]][3] = [WORK,WORK,CARRY,CARRY,CARRY,CARRY,WORK,WORK,WORK,CARRY,CARRY,CARRY,MOVE,MOVE,MOVE];  
        ...
        Types[TWeights[2]] = new Array(numofFields); Types[TWeights[2]][0] = 'upgrader';     Types[TWeights[2]][1] = 5;  Types[TWeights[2]][4] = 999;  Types[TWeights[2]][2] = _.filter(Game.creeps, (creep) => creep.memory.role == 'upgrader' && creep.memory.roomName == RoomName).length;      Types[TWeights[2]][3] = [WORK,WORK,CARRY,CARRY,CARRY,CARRY,WORK,WORK,WORK,CARRY,CARRY,CARRY,MOVE,MOVE,MOVE];
        ...
        Types[TWeights[4]] = new Array(numofFields); Types[TWeights[4]][0] = 'builder';      Types[TWeights[4]][1] = 0;  Types[TWeights[4]][4] = 999;  Types[TWeights[4]][2] = _.filter(Game.creeps, (creep) => creep.memory.role == 'builder' && creep.memory.roomName == RoomName).length;      Types[TWeights[4]][3] = [WORK,WORK,CARRY,CARRY,CARRY,CARRY,WORK,WORK,WORK,CARRY,CARRY,CARRY,MOVE,MOVE,MOVE]; 
        ...
        Types[TWeights[8]] = new Array(numofFields); Types[TWeights[8]][0] = 'gather';       Types[TWeights[8]][1] = 2;  Types[TWeights[8]][4] = 999;  Types[TWeights[8]][2] = _.filter(Game.creeps, (creep) => creep.memory.role == 'gather' && creep.memory.roomName == RoomName).length;   Types[TWeights[8]][3] = [WORK,WORK,WORK,WORK,WORK,WORK,WORK,MOVE,MOVE];
        Types[TWeights[9]] = new Array(numofFields); Types[TWeights[9]][0] = 'haul';         Types[TWeights[9]][1] = 3;  Types[TWeights[9]][4] = 999;  Types[TWeights[9]][2] = _.filter(Game.creeps, (creep) => creep.memory.role == 'haul' && creep.memory.roomName == RoomName).length;  Types[TWeights[9]][3] = [CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,MOVE,MOVE,MOVE,MOVE];
        ...
        if (Game.rooms[RoomName].energyCapacityAvailable < Types[0][4]){
            ...
            toggle = false;
            for (i=0; i <NumofTypes; i++){
                if (Types[i][1] > Types[i][2] && !Game.spawns.Spawn1.spawning && Game.rooms[RoomName].energyAvailable > SpawnRange[RangeID] && Game.rooms[RoomName].energyCapacityAvailable > SpawnRange[RangeID] && !toggle){         
            ...
            for (i=0; i <NumofTypes; i++){
                if (Types[i][1] > Types[i][2] && !Game.spawns.Spawn1.spawning && Game.rooms[RoomName].energyAvailable > Types[i][4] && !toggle){         
